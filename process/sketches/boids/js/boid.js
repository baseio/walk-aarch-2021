class Boid {
  ...
  constructor(boid) {
    ...
    /// forced targetPosition, used with 'groped' mode
    this.forcedTargetPosition = this.position

    /// 
    this.category = boid.color
  ...
  nextPosition() {
    ...
    if( mode === 'selected'){
      if( selectedNode === this.id ){
        var t = new Victor(size.width/2, size.height/2)
        // var diff = t.subtract(this.position);
        // var f = new Victor(diff.x,diff.y);
        // // f.subtract(0.01);
        // f.limitMagnitude(0.5) //this.maxForce);
        // var A = 0.1
        // f.divide({x:A, y:A})
        // this.applyForce( f, 0.3 );

        // var X = size.width / 2
        // var Y = size.width / 2
        // this.position.x = X - (this.position.x - X) * 0.9
        // this.position.y = size.height / 2

        new TWEEN.Tween(this.position).to(t, 250, TWEEN.Easing.Circular.InOut).start()
    ...
    if( mode === 'grouped' ){
      var t = this.forcedTargetPosition.clone() //new Victor(100, 100)
      var diff = t.subtract(this.position);
      var f = new Victor(diff.x,diff.y);
      f.subtract(this.velocity);
      // if( !selectedNode ){
        f.limitMagnitude(this.maxForce);
      // }
