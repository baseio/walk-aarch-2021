...
export class CircleSprite {
	constructor(parent, i, userdata, normalTexture, hoverTexture){
		...
		const circleTranslateArray = [1.0, 0.0, 0.0];
		const circleColorArray = [1.0, 0.0, 0.0, 0.1]; // stroke color
		...
		if( i === 0 ) console.log(this.material);
	...
	update(){
		...
		if( window.app.mode === 'free' ){
			...
			// this.material.uniforms.outer.value = this.r-0.1
			// this.material.uniforms.inner.value = this.r-0.2
		...
		// this.material.setAttribute( 'scale', new InstancedBufferAttribute( new Float32Array([this.r]), 1 ));
	}
}